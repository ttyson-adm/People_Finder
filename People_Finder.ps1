...
foreach ($a in $Addresses | select -ExpandProperty Street) {
    ...
    $json2 = Invoke-WebRequest -Uri "https://www.melissa.com/v2/lookups/addresssearch/?number=&street=$a+$road_type&city=$City&state=TX&zip=&freeForm=Pompano+%0D%0APantego+TX&fmt=json&id=lLqW0h1QWDvdbhZxF0rJy0**nSAcwXpxhQ0PC2lXxuDAZ-**" -Method GET -Headers $header
